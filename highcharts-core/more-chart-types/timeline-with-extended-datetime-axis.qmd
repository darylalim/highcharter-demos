---
title: "Timeline Chart with Extended Date Time Axis Demo"
---

This is a demo of a timeline chart with an extended `datetime` axis.

Highcharts Docs: [Events tied to a `datetime` axis](https://www.highcharts.com/docs/chart-and-series-types/timeline-series#events-tied-to-a-datetime-axis)

Highcharts Samples: [Timeline `datetime` axis](https://jsfiddle.net/gh/get/library/pure/highcharts/highcharts/tree/master/samples/highcharts/series-timeline/datetime-axis)
...
options(
  highcharter.chart = list(
    accessibility = list(
      enabled = TRUE,
      linkedDescription = "A chart illustrating the key moments of space exploration. The chart is extended by further events, and it shows real-time dates distributed along a `datetime` axis."
    )
  )
)
```
...
# Timeline extended with `datetime` axis

```{r}
#| label: timeline-with-extended-datetime-axis
